...
export function createReviewScenario(initial = {}) {
  let state = { ...DEFAULT_SCENARIO, ...initial, deadline: DEADLINE };
  ...
  return {
    ...
    update(patch) {
      state = { ...state, ...patch, deadline: DEADLINE };
...
.review-model-widget {
  --rm-primary: #087e8b;
  --rm-baseline-color: #707070;
  --rm-review-color: #b5571c;
  --rm-completion-color: #8d647d;
  --rm-border: rgba(8, 126, 139, .28);
  --rm-surface: rgba(8, 126, 139, .05);
  color: inherit;
  font-family: inherit;
  min-width: 0;
}
...
.review-model-widget .rm-control input { display: block; width: 100%; margin: .15rem 0; accent-color: var(--rm-primary); }
...
.review-model-widget .rm-review { stroke: var(--rm-primary); }
.review-model-widget .rm-baseline { stroke: var(--rm-baseline-color); stroke-dasharray: 7 4; }
.review-model-widget .rm-manager { stroke: var(--rm-primary); }
.review-model-widget .rm-marker { stroke: currentColor; opacity: .55; stroke-dasharray: 2 4; }
.review-model-widget .rm-marker.rm-review-time { stroke: var(--rm-review-color); opacity: .8; }
.review-model-widget .rm-marker.rm-completion-time { stroke: var(--rm-completion-color); opacity: .8; }
.review-model-widget .rm-threshold { stroke: var(--rm-completion-color); stroke-dasharray: 8 3 2 3; }
...
.review-model-widget .rm-swatch { display: inline-block; width: 1.65rem; flex: 0 0 1.65rem; border-top: 3px solid var(--rm-primary); }
.review-model-widget .rm-swatch.rm-baseline { border-color: var(--rm-baseline-color); border-top-style: dashed; }
.review-model-widget .rm-swatch.rm-review-time { border-color: var(--rm-review-color); border-top-style: dashed; }
.review-model-widget .rm-swatch.rm-completion-time { border-color: var(--rm-completion-color); border-top-style: dashed; }
.review-model-widget .rm-swatch.rm-threshold { border-color: var(--rm-completion-color); border-top-style: dotted; }
.review-model-widget .rm-summary { border-left: 3px solid var(--rm-primary); padding: .45rem .65rem; margin: .65rem 0; background: var(--rm-surface); }
...
.review-model-widget .rm-metric { min-width: 0; padding: .75rem; border: 1px solid var(--rm-border); border-radius: .35rem; background: var(--rm-surface); }
...
.review-model-widget .rm-point-primary { fill: var(--rm-primary); }
.review-model-widget .rm-point-review { fill: var(--rm-review-color); }
.review-model-widget .rm-point-completion { fill: var(--rm-completion-color); }
.quarto-dark .review-model-widget,
[data-bs-theme="dark"] .review-model-widget {
  --rm-primary: #62cbd3;
  --rm-baseline-color: #b7c0c7;
  --rm-review-color: #efa069;
  --rm-completion-color: #d6a9ca;
  --rm-border: rgba(255, 255, 255, .22);
  --rm-surface: rgba(255, 255, 255, .055);
}
...
function addSlider(panel, { key, label, min, max, step, value, decimals = 1 }, onChange) {
  ...
  input.addEventListener("input", () => {
    const nextValue = format(input.valueAsNumber, decimals);
...
function syncSlider(slider, { value, minimum, maximum }) {
  ...
  const nextValue = format(slider.input.valueAsNumber, slider.decimals);
  slider.output.value = nextValue;
  slider.output.textContent = nextValue;
}
...
function addPoint(frame, time, value, className) {
...
export function renderWorkerModel(store = createReviewScenario()) {
  ...
  sliders.totalWork = addSlider(panel,
    { key: "totalWork", label: "表示する総実作業 W", min: 5, max: DEADLINE, step: 1,
  ...
  for (const control of [
    { key: "qbar", label: "基準品質の上限 q̄", min: 20, max: 100, step: 1,
      decimals: 0 },
    { key: "k", label: "初期品質改善 k", min: 0.05, max: 1, step: 0.01,
      decimals: 2 },
  ...
  panel.root.append(htmlElement("p", "rm-note",
    `横軸は1つのタスクに投入する累積実作業 s です。総実作業 W は表示範囲の終点、縦の点線と丸印はレビューまでの実作業量 x を示します。共通デッドラインは T=${DEADLINE} です。初期表示は W=25、q̄=80、k=0.25、h=4、x=6 です。`));
  ...
  redraw = responsiveDrawing(panel, width => {
    ...
    const description = `累積実作業0から${format(state.totalWork)}。レビュー位置${format(state.reviewWork)}、レビュー時の品質${format(reviewQuality)}、総実作業${format(state.totalWork)}での品質${format(finalQuality)}。レビュー時に品質は連続し、その直後の限界品質改善量が上がります。`;
    ...
    addPoint(frame, state.reviewWork, reviewQuality, "rm-point-primary");
    ...
    const rows = [
      ...
      { text: `レビュー直前・直後の限界品質改善量：${format(beforeSlope, 2)} → ${format(afterSlope, 2)}（品質／作業時間）。品質はレビュー時点で連続です。` },
    ];
    if (state.qbar <= 50) {
...
export function renderManagerModel(store = createReviewScenario()) {
  ...
  sliders.completionTime = addSlider(panel,
    { key: "completionTime", label: "最終レビュー時刻 c", min: state.reviewTime + 0.5,
      max: DEADLINE, step: 0.5, value: state.completionTime },
  ...
  panel.root.append(htmlElement("p", "rm-note",
    `a は時刻0の期待品質であり、直線を上下に動かします。b はカレンダー時刻1単位あたりの期待品質の増加量であり、直線の傾きを変えます。横軸はカレンダー時刻 t、橙色の印は中間レビュー時刻 τ、紫色の印は最終レビュー時刻 c を表します。共通デッドラインは T=${DEADLINE} です。作業休止中も期待品質は時間とともに変化します。初期表示は a=20、b=3、τ=6、c=25 です。`));
  ...
  redraw = responsiveDrawing(panel, width => {
    ...
    const maximum = Math.max(120,
      Math.ceil(managerExpectation(DEADLINE, manager) / 20) * 20);
    const description = `カレンダー時刻0から${DEADLINE}。マネージャーの期待品質は${state.a}+${state.b}t。中間レビュー時刻${format(state.reviewTime)}で${format(reviewExpectation)}、最終レビュー時刻${format(state.completionTime)}で${format(completionExpectation)}。`;
    const frame = chartFrame(panel, { width, horizon: DEADLINE, maximum,
      title: "マネージャーの期待品質",
      description, xLabel: "カレンダー時刻 t", yLabel: "期待品質 e(t)" });
    addCurve(frame, DEADLINE,
    ...
    addPoint(frame, state.reviewTime, reviewExpectation, "rm-point-review");
    addPoint(frame, state.completionTime, completionExpectation, "rm-point-completion");
    panel.chart.replaceChildren(frame.svg);
    setSummary(panel, [
  ...
  attachSubscription(panel, store, nextState => {
    ...
    syncSlider(sliders.completionTime,
      { value: state.completionTime, minimum: state.reviewTime + 0.5, maximum: DEADLINE });
    redraw();
  });
  return panel.root;
}
